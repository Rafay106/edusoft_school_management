...
topicRouter.route("/").post(LC.addTopic).get(LC.getTopics);
topicRouter
  .route("/:id")
  .get(LC.getTopic)
...
topicOverviewRouter.route("/").get(LC.getTopicOverview);

// 4 lessonplan
const lesson_planRouter = express.Router();
lesson_planRouter
  .route("/")
  .post(upload(uploadPaths.lesson_plan).single("file"), LC.addLessonPlan);

lessonPlanRouter.use("/lesson", lessonRouter);
